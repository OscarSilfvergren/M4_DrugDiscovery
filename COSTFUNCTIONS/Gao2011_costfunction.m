function [Cost_Model] = Gao2011_costfunction(Gao2011Rat_data,GaoExenatideConcentrations_data,OptimisedParameters,Unoptimised_params,modelName,sNames, pNames,initialvalues,AmountParameterOpti,SimulatePD,fid)

model      = str2func(modelName);

%% Create flexibility for both PS and MCMC

if size(OptimisedParameters,1) > 1 %% flipping the parametervector back to a row vector when running MCMC
OptimisedParameters = OptimisedParameters';
else
    if any(OptimisedParameters < 0)
...
CombinedParameters(13:62)    = OptimisedParameters(1:50);
...
% Aid optimisation so EC50 is lower than IC50
if SimulatePD == 1
...
Cost_Model =  Cost_PK + Cost_PD_G + Cost_PD_I + Cost_Model_Qualitativedemands;
